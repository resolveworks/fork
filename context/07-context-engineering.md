...
The git layer mirrors this: a feature branch per plan (`plan/<slug>`) keeps all step commits organized in one place. The reviewer reads each step's diff as it lands; the human can browse the full branch; merge or revert as a unit. Without a branch, step commits scatter across the log and the plan's full diff is harder to reconstruct.
